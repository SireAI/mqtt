...
public class MqttClient extends ConnectStateCallBack.Stub implements ServiceConnection, IMLocalServiceImpl.ClientReceiver, ModeListener.ICallBack {
    ...
    private MqttClient(Context context, Converter.Factory dataConverter, byte qos, boolean openLog, PushCallBack pushCallBack) {
        if (!inited) {
            inited = true;
    ...
    public <T> void publish(String topic, T payload, MessageCallBack messageCallBack) {
        if (imLocalService instanceof IMLocalService) {
            ((IMLocalServiceImpl) imLocalService).enqueueTask(Task.create(SERIALIZE_REQUEST, Publish.create(topic, payload, messageCallBack)));
        }
    }
    ...
    @MainThread
    @Override
    public void onOperationCallBack(boolean success, int id) {
    ...
    @WorkerThread
    @Override
    public void onRequestSerialized(Publish<byte[]> publish) {
        publish(publish.getTopic(), publish.getPayload(), defaultQos, publish.getMessageCallBack());
    }
    ...
    public static class Builder {
        ...
        public MqttClient build() {
            ...
            if (instance == null) {
                instance = new MqttClient(app, dataConverter, qos,openLog, pushCallBack);
            }
            return instance;
        }
    }
